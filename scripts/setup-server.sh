...
create_app_directory() {
    ...
    APP_DIR="/root/expense-tracker-claude"

    # Create directory with proper permissions (already exists, just ensure subdirs)
    mkdir -p $APP_DIR/logs
    mkdir -p $APP_DIR/backups

    log "Application directory configured at $APP_DIR"
}
...
/root/expense-tracker-claude/logs/*.log {
    daily
    missingok
    rotate 7
    compress
...
Type=simple
User=root
WorkingDirectory=/root/expense-tracker-claude
...
display_summary() {
    ...
    info "  ✅ Application directory: /root/expense-tracker-claude"
    ...
    info "  2. Navigate to /root/expense-tracker-claude"
    info "  3. Ensure your docker-compose.prod.yml file is configured"
    info "  4. Set up your environment variables"
    info "  5. Run: docker-compose -f docker-compose.prod.yml up -d"
    echo ""
